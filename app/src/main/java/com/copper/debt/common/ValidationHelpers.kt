...
fun arePasswordsSame(password: String, repeatPassword: String) =
    isPasswordValid(password) && isPasswordValid(repeatPassword) && password == repeatPassword

fun isValidDebt(text: String, sum: Double, debts: List<Double>): Boolean {
    return (sum - debts.sum() <= 0.01) && (sum > 0) && text.isNotBlank()
}